...
public class VollyHelperNew {
    ...
    public static boolean isSuccess(int errCode) {
        if (errCode == HttpConfig.RESULT_SUCCESS) {
            return true;
        }
        return false;
    }
        ...
        JsonObjectRequest json = new JsonObjectRequest(method, url, jo,
                new Listener<JSONObject>() {
                    ...
                    public void onResponse(JSONObject response) {
                        ...
                        int errCode = response.optInt(HttpConfig.RESULT_KEY, -1);
                        if (errCode == HttpConfig.RESULT_SUCCESS) {
                            /* 如果需要弹出且成功信息不为空 */
                            if (isShowSuccess) {
                                if (infoSuccess != null) {
    ...
    private String checkResponse(JSONObject response, Handler handler, int what) {
        ...
        String info = "";
        try {
            result = response.optInt(HttpConfig.RESULT_KEY, -1);
            if (result == -1) {/* 如果为空，则直接 */
                info = "对不起，服务器返回数据为空......";
                if (handler != null) {
                    handler.obtainMessage(HttpConfig.WHAT_ERROR_HTTP, result)
                            .sendToTarget();
                }
            } else if (HttpConfig.RESULT_LOGIN_OUT_OF_DATE.equals(result)
                    || HttpConfig.WHAT_TOKEN_EXPIRED == result) {

            } else if (result == HttpConfig.RESULT_SUCCESS) {
				/* 错误信息一定要提示，但是成功信息不一定要提示，即使提示，成功信息也各不相同 */
                if (handler != null) {
                    handler.obtainMessage(what, response).sendToTarget();
                }
            } else {/* 如是失败，则直接将失败信息传回 */
                info = response.get(HttpConfig.RESULT_MESSAGE_KEY).toString();
                if (handler != null) {
                    if (result == HttpConfig.WHAT_ERROR_HTTP) {
                        handler.obtainMessage(HttpConfig.WHAT_ERROR_HTTP, result)
                                .sendToTarget();
                    } else {
    ...
    private void sendError(Handler handler, VolleyError error) {
        handler.obtainMessage(HttpConfig.WHAT_ERROR_HTTP, "对不起，网络请求错误...")
                .sendToTarget();
    }
