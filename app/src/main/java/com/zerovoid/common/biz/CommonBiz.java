...
import com.zerovoid.http.HttpConfig;
...
public class CommonBiz {
     ...
     * @param response
     * @param errCode
     * @param isShowErrorInfo 是否显示98型错误
     * @return
     */
    public static boolean isSuccess(JSONObject response, int errCode,
                                    boolean isShowErrorInfo) {
        if (errCode == HttpConfig.RESULT_SUCCESS && response != null) {
            return true;
            ...
            String message = "";
            try {
                message = response.getString(HttpConfig.RESULT_MESSAGE_KEY);
            } catch (JSONException e) {
                e.printStackTrace();
            }
